...
inline void fft::fft_bit_reverse(unsigned int n, unsigned int bits, plm_t &plm_local)
{
	unsigned int i, s, shift;
        ...
        for (i = 0; i < n; i++) {
            ...
            wait();
            t1_real = plm_local.data[2 * i];
            t1_imag = plm_local.data[2 * i + 1];
            wait();
            t2_real = plm_local.data[2 * r];
            t2_imag = plm_local.data[2 * r + 1];

            if (i < r) {
                HLS_PROTO("bit-rev-memread");
                HLS_BREAK_DEP(plm_local.data);
                wait();
                plm_local.data[2 * i] = t2_real;
                plm_local.data[2 * i + 1] = t2_imag;
                wait();
                plm_local.data[2 * r] = t1_real;
                plm_local.data[2 * r + 1] = t1_imag;
            }
        }
}
