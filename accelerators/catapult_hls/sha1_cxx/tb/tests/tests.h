...
int sha1_montecarlo(void)
{
    ...
    for (unsigned t = 0; t < cavp.tot_tests; ++t)
    {
        ...
        for (unsigned j = 3; j < 1003; ++j)
        {
            ...
            const unsigned sha1_in_bytes = 3 * 160 / 8;
            ...
            for (unsigned b = 0; b < sha1_batch; b++) {
               for (unsigned i = 0; i < sha1_in_bytes; i+=8) {
                    ...
                    data_t d0 = (i+0 < sha1_in_bytes) ? buffer_in3[i+0] : data_t(0);
                    data_t d1 = (i+1 < sha1_in_bytes) ? buffer_in3[i+1] : data_t(0);
                    data_t d2 = (i+2 < sha1_in_bytes) ? buffer_in3[i+2] : data_t(0);
                    data_t d3 = (i+3 < sha1_in_bytes) ? buffer_in3[i+3] : data_t(0);
                    data_t d4 = (i+4 < sha1_in_bytes) ? buffer_in3[i+4] : data_t(0);
                    data_t d5 = (i+5 < sha1_in_bytes) ? buffer_in3[i+5] : data_t(0);
                    data_t d6 = (i+6 < sha1_in_bytes) ? buffer_in3[i+6] : data_t(0);
                    data_t d7 = (i+7 < sha1_in_bytes) ? buffer_in3[i+7] : data_t(0);
        ...
        // Validation
        test_passed += eval_cavp(&cavp, buffer_out, 160 / 8, t, SHA_MONTECARLO, SHA1_MONTECARLO_VERBOSE);
