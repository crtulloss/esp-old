...
LOAD_INPUT_DATA_LOOP:
            for (uint16_t i = 0; i < len; i++) {
                ...
                if (ping) {
                    ESP_REPORT_TIME(VOFF, sc_time_stamp(), "Load load(): plm0_in.data[%d] := %llX", i, data);
                } else {
                    ESP_REPORT_TIME(VOFF, sc_time_stamp(), "Load load(): plm1_in.data[%d] := %llX", i, data);
                }
            }
            if (ping) {
...
STORE_OUTPUT_DATA_LOOP:
            for (uint16_t i = 0; i < len; i++) {
                ...
                if (ping) {
                    ESP_REPORT_TIME(VOFF, sc_time_stamp(), "Store store(): plm0_out.data[%d] -> %llX", i, data);
                } else {
                    ESP_REPORT_TIME(VOFF, sc_time_stamp(), "Store store(): plm1_out.data[%d] -> %llX", i, data);
                }
...
void dummy::compute_kernel()
{
    ...
    uint32_t batch;
    {
        ...
        ESP_REPORT_TIME(VOFF, sc_time_stamp(), "Compute config(): tokens = %llu, batch = %llu", ESP_TO_UINT64(tokens), ESP_TO_UINT64(batch));
    }
...
COMPUTE_BATCH_LOOP:
    for (unsigned n = 0; n < batch; n++)
    {
        ...
        ESP_REPORT_TIME(VOFF, sc_time_stamp(), "Compute compute(): batch # %d start...", n+1);
...
COMPUTE_TOKENS_LOOP:
        for (int b = tokens; b > 0; b -= PLM_SIZE)
        {
            ...
            ESP_REPORT_TIME(VOFF, sc_time_stamp(), "Compute compute(): ping = %d", ping);

            uint32_t len = b > PLM_SIZE ? PLM_SIZE : b;

            plm_t plm_local;
            if (ping) {
                plm_local = plm0_in.read();
            } else {
        ...
        ESP_REPORT_TIME(VOFF, sc_time_stamp(), "Compute compute(): batch # %d done!", n+1);
    }
