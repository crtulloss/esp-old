...
#include "vitdecoder.hpp"
#include "vitdecoder_directives.hpp"

// Functions

#include "vitdecoder_functions.hpp"

// Processes

void vitdecoder::load_input()
{
...
#endif
            // printf("DMA_WORD_PER_BEAT = %u\n", DMA_WORD_PER_BEAT);
            // printf("length = %u\n", length);
            //printf("Doing read of rem = %u for %u per xfer\n", length, PLM_IN_WORD);
            // Chunking
...
void vitdecoder::compute_kernel()
{
    // Reset
    {
    ...
    int32_t data_bits;
    {
        ...
        ntraceback = config.ntraceback;
        data_bits = config.data_bits;
        //printf("load-config set cbps = %u, ntraceback = %u, data_bits = %u\n", cbps, ntraceback, data_bits);
        //printf("   form config  cbps = %u, ntraceback = %u, data_bits = %u\n", config.cbps, config.ntraceback, config.data_bits);
    }
    ...
    bool ping = true;
    {
        for (uint16_t b = 0; b < 1; b++)
        {
            ...
            //printf("Finished compute_load_handshake\n");
            {
