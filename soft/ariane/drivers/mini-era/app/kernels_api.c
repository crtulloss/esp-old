...
unsigned bad_decode_msgs = 0; // Total messages decoded incorrectly during the full run
  ...
  #define VIT_DEVNAME	"/dev/vitbfly2.0"

  int vitHW_fd;
  contig_handle_t vitHW_mem;  // Pointer to contiguous memory
  uint8_t*  vitHW_lmem; // Pointer to my local view of allocated contig space
  const size_t vitHW_size = 6 * 64 * sizeof(unsigned char);
  const size_t out_vitHW_size = 4 * 64 * sizeof(unsigned char);
  struct vitbfly2_access vitHW_desc;
#endif

#ifdef HW_FFT
  // These are FFT Harware Accelerator Variales, etc.
...
status_t init_rad_kernel(char* dict_fn)
{
  ...
  if (fftHW_lmem == NULL) {
    fprintf(stderr, "Error: cannot allocate %zu contig bytes", fftHW_input_size);
    exit(EXIT_FAILURE);
  }
...
status_t init_vit_kernel(char* dict_fn)
{
  ...
  printf("Allocate hardware buffer of size %zu\n", vitHW_size);
  vitHW_lmem = contig_alloc(vitHW_size + out_vitHW_size + 128, &vitHW_mem);
  if (vitHW_lmem == NULL) {
