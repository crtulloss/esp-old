...
bool thread_is_p2p(esp_thread_info_t *thread)
{
     switch (thread->type) {
        // <<--esp-p2p-thread-->>
		case mindfuzz :
			return (thread->desc.mindfuzz_desc.esp.p2p_store
				|| thread->desc.mindfuzz_desc.esp.p2p_nsrcs);
         case fftaccelerator :
...
void *accelerator_thread( void *ptr )
{
	...
	// <<--esp-ioctl-->>
	case mindfuzz :
		rc = ioctl(info->fd, MINDFUZZ_IOC_ACCESS, info->desc.mindfuzz_desc);
		break;
	case fftaccelerator :
...
static void esp_config(esp_thread_info_t* cfg[], unsigned nthreads, unsigned *nacc)
{
	...
	for (i = 0; i < nthreads; i++) {
        ...
        for(j = 0; j < len; j++){
            ...
            // <<--esp-prepare-->>
		case mindfuzz :
			esp_prepare(&info->desc.mindfuzz_desc.esp, handle, policy);
			break;
            case fftaccelerator :
