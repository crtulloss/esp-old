...
#define SLD_SOFTMAX_SYSC 0x050
#define DEV_NAME "sld,softmax_sysc"
...
static void softmax_sysc_sw(float *input, float *output)
{
    float exp_in[size];
    float sum_exp = 0;
    unsigned i;
...
static void init_buf (token_t *in, token_t * gold)
{
	...
	for (i = 0; i < batch; i++)
    {
        ...
        softmax_sysc_sw(in_local_gold + (i*size), out_local_gold + (i*size));
    }
...
int main(int argc, char * argv[])
{
	...
	ndev = probe(&espdevs, SLD_SOFTMAX_SYSC, DEV_NAME);
	if (ndev == 0) {
#ifndef __riscv
		printf("softmax_sysc not found\n");
#else
		print_uart("softmax_sysc not found\n");
#endif
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
		...
		/* <<--regs-config-->> */
		iowrite32(dev, SOFTMAX_SYSC_BATCH_REG, batch);
